...
export const ConsultationView: React.FC<ConsultationViewProps> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!isAnonymous && !studentName.trim()) {
      setError('Nama siswa wajib diisi (atau centang Kirim sebagai Anonim).');
      return;
    }
    ...
    try {
      ...
      setSuccessMsg('Curhatmu berhasil dikirimkan ke Guru BK SMKN 2 Godean.');
      setTopic('');
      setMessage('');
      setUrgency('sedang');
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-6 bg-white rounded-3xl p-6 sm:p-7 border border-slate-200 shadow-xs space-y-4">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 p-3.5 bg-emerald-50/60 border border-emerald-200/80 rounded-2xl">
              <div>
                <label className="block text-xs font-bold text-slate-800 mb-1 flex items-center gap-1.5">
                  <UserCheck className="w-3.5 h-3.5 text-emerald-600" />
                  <span>Nama Siswa {!isAnonymous && '*'}</span>
                </label>
                <input
                  type="text"
                  ...
                  placeholder={isAnonymous ? 'Disamarkan (Anonim)' : 'Nama lengkapmu'}
                  ...
                  className={`w-full px-3 py-2 text-xs sm:text-sm rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-600 ${
                    isAnonymous ? 'bg-slate-100 text-slate-400 cursor-not-allowed' : 'bg-white text-slate-800'
                  }`}
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-3 py-2 text-xs sm:text-sm rounded-xl border border-slate-300 focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-600 bg-white text-slate-800"
                />
              </div>
            </div>
